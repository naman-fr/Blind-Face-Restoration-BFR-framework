...
from bfr_framework.datapipe.datasets import BaseDataFolder
from bfr_framework.basicsr.archs.rrdbnet_arch import RRDBNet
from bfr_framework.basicsr.utils.realesrgan_utils import RealESRGANer
from bfr_framework.facelib.utils.face_restoration_helper import FaceRestoreHelper
...
class BaseSampler:
    def __init__(self, configs, im_size=512, use_fp16=False):
        ...
        if hasattr(torch, 'compile') and os.getenv('USE_TORCH_COMPILE', '0') == '1':
            logger.info("Enabling torch.compile for model speedup...")
...
class DifFaceSampler(BaseSampler):
    ...
    def build_model(self):
        ...
        model_ir = obj(**self.configs.model_ir.params)
        if torch.cuda.is_available():
        ...
        if 'net_hq' in self.configs:
            ...
            self.net_hq = net_hq.cuda() if torch.cuda.is_available() else net_hq
        ...
        if 'net_lq' in self.configs:
            ...
            self.net_lq = net_lq.cuda() if torch.cuda.is_available() else net_lq
    ...
    def sample_func_ir_unaligned(
        ...
        def _process_batch(cropped_faces_list):
            ...
            cropped_face_t = torch.cat(
                    ...
                    axis=0)
            if torch.cuda.is_available():
                cropped_face_t = cropped_face_t.cuda()
            cropped_face_t = cropped_face_t / 255.
        ...
        if num_cropped_face == 0:
            restored_faces = []
        elif num_cropped_face > micro_bs:
            restored_faces = []
    ...
    def inference(
        ...
        if in_path.is_dir():
                ...
                for data in tqdm(dataloader):
                    ...
                    if ind_start < data['image'].shape[0]:
                        ...
                        if (not cond_kwargs is None) and 'mask' in cond_kwargs and mask_back:
                            mask_cuda = mask.cuda() if torch.cuda.is_available() else mask
                            y0_cuda = current_y0.cuda() if torch.cuda.is_available() else current_y0
                            sample_cuda = sample.cuda() if torch.cuda.is_available() else sample
                            sample = sample_cuda * mask_cuda + y0_cuda * (1-mask_cuda)
            ...
            if self.configs.aligned:
                ...
                if (not cond_kwargs is None) and 'mask' in cond_kwargs and mask_back:
                    mask_cuda = mask.cuda() if torch.cuda.is_available() else mask
                    y0_cuda = y0.cuda() if torch.cuda.is_available() else y0
                    sample_cuda = sample.cuda() if torch.cuda.is_available() else sample
                    sample = sample_cuda * mask_cuda + y0_cuda * (1-mask_cuda)
